...
async def test_multi_tenant_isolation(db_password, session_mgr, checker):
    ...
    agent_lyon = TokenClaims(
        user_id="agent-lyon", tenant_id="prison-lyon-001", level=3,
        roles=["site_operator"], permissions=["site:events:read"],
        exp=now + timedelta(minutes=15), iat=now
    )
    
    conn = psycopg2.connect(
        host="localhost", database="zynaxia",
        user="zynaxia_app", password=db_password
    )
    cur = conn.cursor()
...
async def test_session_revocation_immediate(session_mgr):
    ...
    claims = TokenClaims(
        user_id="test-user", tenant_id="test-tenant", level=3,
        roles=[], permissions=[],
        exp=now + timedelta(minutes=15), iat=now
    )
    
    session = await session_mgr.create_session(
        claims.user_id, claims.tenant_id, claims
    )
