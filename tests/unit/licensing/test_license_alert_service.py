...
from src.licensing.license_alert_service import LicenseAlertService, AlertLevel, AlertChannel, AlertConfig
...
def alert_service(license_cache, audit_emitter):
    ...
    return LicenseAlertService(license_cache=license_cache, audit_emitter=audit_emitter)
...
class TestLIC030Compliance:
    ...
    @pytest.mark.asyncio
    async def test_LIC_030_j60_alert_not_pending_at_65_days(
        self, alert_service, license_cache, license_expiring_65_days
    ):
...
class TestAlertLogic:
    ...
    @pytest.mark.asyncio
    async def test_check_and_send_alerts_multiple_pending(
        self, alert_service, license_cache, license_expiring_5_days, audit_emitter
    ):
...
class TestChannelSending:
    ...
    async def test_format_alert_message(self, alert_service, license_cache, license_expiring_5_days):
        ...
        formatted = alert_service._format_alert_message(message_template, "site-test", license_expiring_5_days, 5)
