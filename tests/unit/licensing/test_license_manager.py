...
class TestLIC006Compliance:
    ...
    @pytest.mark.asyncio
    async def test_LIC_006_issue_requires_cloud_mode(
        self, crypto_provider, audit_emitter, blockchain_anchor, valid_license_config
    ):
...
class TestLIC061Compliance:
    ...
    async def test_LIC_061_revoke_requires_quorum(self, license_manager, valid_license_config):
        ...
        single_signature = [
            Signature(signer_id="admin-1", signature="sig1", timestamp=datetime.now(timezone.utc), action="revoke")
        ]
    ...
    @pytest.mark.asyncio
    async def test_LIC_061_revoke_with_sufficient_quorum(
        self, license_manager, valid_license_config, sample_signatures, crypto_provider
    ):
...
class TestLIC090Compliance:
    ...
    async def test_LIC_090_issue_emits_audit(self, license_manager, valid_license_config, audit_emitter):
        ...
        # Vérifier arguments: emit_event(type, user, tenant, action, resource, metadata)
    ...
    @pytest.mark.asyncio
    async def test_LIC_090_revoke_emits_audit(
        self, license_manager, valid_license_config, sample_signatures, audit_emitter, crypto_provider
    ):
...
class TestInputValidation:
    ...
    async def test_issue_invalid_modules_fails(self, license_manager):
        ...
        invalid_config = LicenseConfig(
            site_id="site-123", modules=["invalid_module", "another_invalid"], duration_days=90, issuer_id="issuer-123"
        )
