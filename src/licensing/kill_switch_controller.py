...
╚═══════════════════════════════════════════════════════════╝
    ...
    def __init__(self, audit_emitter: AuditEmitter, blockchain_anchor: BlockchainAnchor):
        """
        Args:
    ...
    async def get_status(self, site_id: str) -> Dict[str, Any]:
        ...
        """
        if not self.is_active(site_id):
            return {"active": False, "site_id": site_id}
...
╚═══════════════════════════════════════════════════════════╝
    ...
    def get_kill_switch_stats(self) -> Dict[str, Any]:
        ...
        if total_active == 0:
            return {"total_active_switches": 0, "oldest_switch_hours": 0, "total_bypass_attempts": 0}

        # Calculs statistiques
        now = datetime.now(timezone.utc)
        durations = [(now - switch.activated_at).total_seconds() / 3600 for switch in self._active_switches.values()]

        total_bypass_attempts = sum(switch.bypass_attempts for switch in self._active_switches.values())
