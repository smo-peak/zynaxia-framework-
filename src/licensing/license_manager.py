...
class LicenseManager(ILicenseManager):
        ...
        try:
            ...
            if license.revoked:
                return ValidationResult(
                    ...
                    reason=f"Licence révoquée: {license.revoked_reason}"
                )
            ...
            if not self._verify_license_signature(license):
                return ValidationResult(
                    ...
                    reason="Signature ECDSA-P384 invalide"
                )
            ...
            if not self._validate_license_structure(license):
                return ValidationResult(
                    ...
                    reason="Structure licence invalide"
                )
    ...
    def _verify_license_signature(self, license: License) -> bool:
        ...
        try:
            ...
            # Vérifier signature
            return self.crypto_provider.verify_signature(
    ...
    def get_license_stats(self) -> Dict[str, Any]:
        ...
        expiring_soon = sum(
            ...
            if not lic.revoked and (lic.expires_at - now).days <= 30
        )
