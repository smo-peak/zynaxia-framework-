...
class LicenseAlertService:
    ...
    ALERT_SCHEDULE: List[AlertConfig] = [
        ...
        AlertConfig(
            ...
            message_template="Licence ZYNAXIA expire dans 60 jours. Contactez votre intégrateur.",
        ...
        AlertConfig(
            ...
            message_template="🚨 CRITIQUE: Licence expire dans 7 jours. Suspension imminente.",
            integrator_notification=True,
            organization_notification=True,
        ),
        # LIC_034: J-1 tous canaux + webhook
        AlertConfig(
            ...
            message_template="🔥 DERNIÈRE CHANCE: Licence expire DEMAIN. Kill switch imminent.",
            integrator_notification=True,
            organization_notification=True,
        ),
    ]

    def __init__(self, license_cache: ILicenseCache, audit_emitter: AuditEmitter):
        """
        Args:
        ...
        self._integrator_contacts = {"email": "support@zynaxia-integrator.com", "phone": "+33123456789"}
    ...
    async def get_alert_status(self, site_id: str) -> Dict[str, Any]:
        ...
        for alert_config in self.ALERT_SCHEDULE:
            if alert_config.days_before >= days_until_expiry and alert_config.days_before not in sent_alerts:
    ...
    async def _send_alert(
        self, site_id: str, license: License, alert_config: AlertConfig, days_until_expiry: int
    ) -> None:
        """
        ...
        message = self._format_alert_message(alert_config.message_template, site_id, license, days_until_expiry)

        # Envoyer sur chaque canal
        for channel in alert_config.channels:
            await self._send_channel_alert(channel, site_id, license, message, alert_config.level)

    def _format_alert_message(self, template: str, site_id: str, license: License, days_until_expiry: int) -> str:
    ...
    async def _send_channel_alert(
        self, channel: AlertChannel, site_id: str, license: License, message: str, level: AlertLevel
    ) -> None:
    ...
    async def _audit_alert_sent(self, site_id: str, alert_config: AlertConfig, days_until_expiry: int) -> None:
        """Audit envoi alerte."""
    ...
    async def _audit_alert_error(self, site_id: str, alert_config: AlertConfig, error: str) -> None:
