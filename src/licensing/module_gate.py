...
class ModuleGate(IModuleGate):
    ...
    async def update_module_list(self, site_id: str, modules: List[str]) -> None:
        ...
        await self._audit.emit_event(
            ...
            "module_list_updated",
            resource_id=site_id,
            metadata={"new_modules": modules, "module_count": len(modules)},
        )
    ...
    async def get_module_access_stats(self, site_id: str) -> Dict[str, Any]:
        ...
        if not site_id:
            return {"licensed_modules_count": 0, "total_modules_available": 10, "license_coverage_percent": 0.0}
    ...
    async def _audit_access_denied(self, site_id: str, module_id: str, user_id: str, reason: str) -> None:
        """Audit tentative accès refusé (LIC_083)."""
        await self._audit.emit_event(
    ...
    async def _audit_access_granted(self, site_id: str, module_id: str, user_id: str) -> None:
        """Audit accès autorisé (trace positive)."""
        await self._audit.emit_event(
            ...
            resource_id=module_id,
            metadata={"module_id": module_id, "site_id": site_id, "user_id": user_id},
        )

    def _audit_access_denied_sync(self, site_id: str, module_id: str, user_id: str, reason: str) -> None:
    ...
    def _audit_access_granted_sync(self, site_id: str, module_id: str, user_id: str) -> None:
