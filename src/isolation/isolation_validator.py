...
class IsolationValidator(IIsolationValidator):
    ...
    def test_cross_level_isolation(self, child_tenant: str, child_level: int, 
                                 parent_tenant: str, parent_level: int) -> bool:
        """
                ...
                if tenant_a["level"] > tenant_b["level"]:  # A est enfant de B
                    ...
                    try:
                        if not self.test_cross_level_isolation(
                            tenant_a["id"], tenant_a["level"],
                            tenant_b["id"], tenant_b["level"]
                        ):
