...
class BlockchainAnchor(IBlockchainAnchor):
    ...
    async def verify_anchor(self, receipt: AnchorReceipt) -> bool:
        ...
        try:
            ...
            if (
                stored_receipt.blockchain_tx_id != receipt.blockchain_tx_id
                or stored_receipt.block_height != receipt.block_height
            ):
                return False
    ...
    def get_anchor_stats(self) -> dict:
        ...
        recent_count = sum(
            1
            for receipt in self._anchored_hashes.values()
    ...
    async def get_anchor_history(self, hours_back: int = 24) -> list[AnchorReceipt]:
        ...
        recent_receipts = [
            receipt for receipt in self._anchored_hashes.values() if receipt.anchor_timestamp >= cutoff_time
        ]
